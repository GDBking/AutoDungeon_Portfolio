...
public class Setting : MonoBehaviour
{
    ...
    const string KEY_ADVICE = "AdviceOn";

    // 0 = Windowed, 1 = FullScreenWindow(borderless), 2 = ExclusiveFullScreen
    ...
    private void Start()
    {
        ...
        adviceOn = PlayerPrefs.GetInt(KEY_ADVICE, 1) == 1;

        if (SceneManager.GetActiveScene().name == "2.Dungeon")
        {
            // 도움말
            ApplyAdviceState(adviceOn);

            UpdateAdviceButtonText();
    ...
    private void OnEnable()
    {
        ...
        bgmBar.value = PlayerPrefs.GetInt(KEY_BGM, 100);
    ...
    public void SaveBtnClick()
    {
        ...
        PlayerPrefs.SetInt(KEY_ADVICE, adviceOn ? 1 : 0);
        PlayerPrefs.Save();
    }
    ...
    [Header("물음표 도움말")]
    public GameObject adviceIcons;
    public TextMeshProUGUI adviceButtonText;
    ...
    void ApplyAdviceState(bool on)
    {
        if (adviceIcons != null)
            adviceIcons.SetActive(on);
    }

    void UpdateAdviceButtonText()
    {
        if (adviceButtonText == null)
            return;

        adviceButtonText.text = adviceOn ? GetAdviceTurnOffText() : GetAdviceTurnOnText();
    }
    ...
    string GetAdviceTurnOnText()
    {
        string code = GetLocaleCode();

        if (code.StartsWith("ko")) return "켜기";
        if (code.StartsWith("en")) return "Turn On";
        if (code.StartsWith("fr")) return "Activer";
        if (code.StartsWith("it")) return "Attiva";
        if (code.StartsWith("de")) return "Einschalten";
        if (code.StartsWith("es")) return "Activar";
        if (code.StartsWith("ja")) return "オンにする";
        if (code.StartsWith("pt")) return "Ativar";
        if (code.StartsWith("ru")) return "Включить";
        if (code.StartsWith("zh")) return "开启";

        // 기본값 영어
        return "Turn On";
    }

    string GetAdviceTurnOffText()
    {
        string code = GetLocaleCode();

        if (code.StartsWith("ko")) return "끄기";
        if (code.StartsWith("en")) return "Turn Off";
        if (code.StartsWith("fr")) return "Désactiver";
        if (code.StartsWith("it")) return "Disattiva";
        if (code.StartsWith("de")) return "Ausschalten";
        if (code.StartsWith("es")) return "Desactivar";
        if (code.StartsWith("ja")) return "オフにする";
        if (code.StartsWith("pt")) return "Desligar";
        if (code.StartsWith("ru")) return "Выключить";
        if (code.StartsWith("zh")) return "关闭";
